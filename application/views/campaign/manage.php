<!DOCTYPE html>
<html><head>
<title>Flexmailer | Manage Campaigns</title>
<meta charset="UTF-8">
...
<!-- ===================================== END HEADER ===================================== -->
	...
	<!-- 
	 ...
	 <ul class="breadcrumbs alt1">
	 	...
	 	<li><a href="#">Campaign Created.</a></li>
	 </ul>
	 </div>
	 <div class="col_9">
	 	<h4>Manage Campaigns</h4><br/>
	 	<?php
	 	if ($mycampaigns) {
	 		echo('<h6>My Campaigns</h6><table><thead><tr><th>Name</th><th>Actions</th></tr></thead><tbody>');
	 		foreach($mycampaigns as $mycampaign) {
	 			echo('<tr><td>'.$mycampaign['_id'].'</td><td><a href="/job/create/'.$mycampaign['_id'].'" class="button small green">Create Job</a></td></tr>');
	 		}
			echo('</tbody></table>');
	 	} else {
	 		echo('<p>You have not created any campaigns.<br/><a href="/campaign/new" class="button green small">Create a Campaign</a></p>');
	 	}
	 	if ($user->role == 'admin') {
	 		if ($allcampaigns) {
	 			echo('<h6>All Campaigns</h6><table><thead><tr><th>Name</th><th>Actions</th></tr></thead><tbody>');	
	 			foreach ($allcampaigns as $allcampaign) {
	 				echo('<tr><td>'.$allcampaign['_id'].'</td><td><a href="/job/create/'.$allcampaign['_id'].'" class="button small green">Create Job</a></td></tr>');
	 			}
				echo('</tbody></table>');
			}
	 	}
